...
import { frontBox } from './common.ts';
import { KOOPA_PARTS, KOOPA_PROPS, KOOPA_SHELL, KOOPA_TAIL, makeKoopaBase, koopaStrike, koopaBossCommon } from './koopaRig.ts';
import { EARTH, drawBramble } from './koopaKit.ts';
import { TRIO_HOOKS, trioAllies, healKing, treeSafe, burrowAnim, tunnelToNearest, burrowDrawBefore, burrowDrawAfter } from './koopaTrio.ts';
...
const earthAnims = Object.assign(makeKoopaBase(EC), common, {
  ...
  burrow: burrowAnim(EC, { radius: 40, hit: hit(16, 'launch', 3, 7, 26, { status: TANGLE }) }, EARTH.thorn),
...
export const koopaEarth = {
  ...
  hooks: {
    ...
    onHitTaken(f, h, attacker, world) {
      if (treeSafe(f, attacker)) return false;
    ...
    drawBefore: burrowDrawBefore,
    drawAfter: burrowDrawAfter,
    onPhase(f, i, world) {
      if (!world || i === 0) return;
