...
const TYPE_ALIASES = {
  typea: 'brassbound', typeb: 'sootborn', typec: 'stormcrow', brass: 'brassbound', soot: 'sootborn', crow: 'stormcrow', ninth: 'stormcrow',
...
const DEFS = new Map();
for (const d of [...BRASSBOUND, ...SOOTBORN, ...STORMCROWS]) DEFS.set(`${d.type}:${d.variant}`, d);
 ...
 * @param {string} type 'brassbound' | 'sootborn' | 'stormcrow' | 'midboss' | 'boss' | 'midboss2' | 'boss2'
 *   (aliases: typeA, typeB, typeC, grubbik, vane, skree, kestrel)
...
/** Registry list for window.__game.enemyList(): 15 variants + both stages' mid-bosses and bosses. */
const listEntry = (d) => ({ type: d.type, variant: d.variant, name: d.name, role: d.role });
export const ENEMY_LIST = [
  ...BRASSBOUND.map(listEntry), ...SOOTBORN.map(listEntry), ...STORMCROWS.map(listEntry),
...
export const ENEMY_GALLERY = [
  ...BRASSBOUND.map(galleryEntry), ...SOOTBORN.map(galleryEntry), ...STORMCROWS.map(galleryEntry),
...
export { BRASSBOUND, SOOTBORN, STORMCROWS, midboss, boss, midboss2, boss2 };
