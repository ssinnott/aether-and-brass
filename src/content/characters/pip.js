...
//
// Readability (docs/ART_STYLE.md section 0): light skin / red hat / dark leather jacket on the gnome; mid frame grey scaffold with a
// dark iron boiler panel and brass rims; lighter grey upper-arm cylinders over dark iron piston sleeves with light steel rods and
// brass claws; dark iron thigh cylinders over light steel shin rods on slate plate feet. Ground keys use G(): root.y is solved so
// the lowest foot plate sits on the floor (spec.root[1] is an extra sink), so the long piston legs never float.
...
// skin: light warm  hair: dark brown (gnome)  primary: frame grey  sleeve: light grey arm cylinders  secondary: dark iron pistons
// accent: brass  metal: light steel rods  dark: slate foot plates  glow: boiler fire
const PAL = { skin: '#F5E0C8', hair: '#5A3420', primary: '#6B6B75', sleeve: '#9A9AA6', secondary: '#454A58', accent: '#C9A227', metal: '#B8C0CC', dark: '#3A3F4F', glow: '#E86A1E' };
const HAT = '#C74E4E', JACKET = '#7A4E2E', IRON = '#3A3A44', GAUGE = '#59C3A0', STEAM = '#DDE6EC', HOT = '#FFD27A', BROW = '#4A2A1A', LAMP = '#FFC04A', INK = '#1E1A22';
...
function stackPuffs(ctx, rig, x, y, phase) {
  const f = (rig.tick + phase) % 20, k = f / 20, a0 = ctx.globalAlpha, big = rig.vent > 0 ? 1.8 : 1;
  ctx.fillStyle = tones(rig, STEAM).base;
  puff(ctx, x - k * 3, y - k * 14 * big, (1.5 + k * 3.5) * big, a0 * (0.6 - k * 0.5));
  if (f > 10) { const k2 = (f - 10) / 20; puff(ctx, x - k2 * 2, y - k2 * 9 * big, (1.2 + k2 * 2.5) * big, a0 * (0.45 - k2 * 0.4)); }
  ctx.globalAlpha = a0;
}
/** Head: braid chain (behind), skull with a pointed gnome ear, then her little body on the seat with both hands on a brass lever. */
function drawHead(ctx, rig, pose, inf) {
  ...
  ctx.restore();
  // her body under the chin: leather jacket block on the seat, two short arms forward to the lever
  const by = R(r * 0.8);
  celRect(ctx, rig, -7, by, 14, 11, 2, JACKET, 0.36, 0.25);
  celCapsule(ctx, rig, 2, by + 3, R(r * 0.9), by + 7, 2.5, JACKET, 0);
  celBall(ctx, rig, R(r * 1.0), by + 7, 2.5, PAL.skin, false);
  ctx.beginPath(); ctx.rect(R(r * 1.1), by + 1, 3, 8); flat(ctx, rig, PAL.accent);
  celBall(ctx, rig, R(r * 1.1) + 1, by, 2.5, HAT, false);
...
/** Big-feature face (5x4 whites, 2 px brows) with a freckle row under the eyes. */
function drawFaceP(ctx, rig, pose, inf) {
  drawFace(ctx, rig, inf.r, pose.face | 0, { big: true, brow: BROW, eyeY: 1 });
}
/** Red pointed hat with a brass band, leaning back a little (head space, drawn after the face). */
function drawHat(ctx, rig, pose, inf) {
  const r = inf.r, brim = R(-r * 0.5);
...
function drawSeat(ctx, rig) { ctx.beginPath(); ctx.rect(-7, -2, 14, 4); flat(ctx, rig, IRON); }
/** Scaffold torso: frame-grey rounded frame, dark iron panel, brass collar, boiler with a fire window, pressure gauge (needle = meter). */
function drawTorso(ctx, rig, pose, inf) {
  const W = inf.w, H = inf.h, hw = R(W / 2);
  celRect(ctx, rig, -hw, -H, W, H + 2, 3, PAL.primary, 0.36, 0.28);
  ctx.beginPath(); ctx.rect(-hw + 3, -H + 5, W - 6, H - 7); flat(ctx, rig, IRON);
  celBall(ctx, rig, 0, -10, 8, PAL.accent);
  celBall(ctx, rig, 0, -25, 5, PAL.accent, false);
  if (rig.override) return;
  const tb = tones(rig, PAL.accent), ti = tones(rig, IRON);
  // brass collar + four corner rivets
  ctx.fillStyle = tb.base; ctx.fillRect(-hw + 1, -H + 1, W - 2, 3);
  ctx.fillStyle = tb.sh; ctx.fillRect(-hw + 1, -H + 3, W - 2, 1);
  ctx.fillStyle = tb.base; ctx.fillRect(-hw + 1, -H + 6, 3, 3); ctx.fillRect(hw - 4, -H + 6, 3, 3); ctx.fillRect(-hw + 1, -4, 3, 3); ctx.fillRect(hw - 4, -4, 3, 3);
  ...
  ctx.fillStyle = rig.col('#241a1c'); ctx.fillRect(-4, -13, 8, 7);
  ctx.fillStyle = rig.col(PAL.glow); ctx.fillRect(-3, -12, 6, 5);
  ctx.fillStyle = rig.col(HOT); if (hot) ctx.fillRect(-2, -11, 4, 3); else ctx.fillRect(-1, -10, 2, 2);
  // gauge: green dial, needle sweeps -120..120 deg with the meter (rig.meterFrac, set by hooks.onUpdate)
  ctx.fillStyle = rig.col(GAUGE); ctx.beginPath(); ctx.arc(0, -25, 3.5, 0, TAU); ctx.fill();
  const fr = rig.meterFrac != null ? rig.meterFrac : 0.35;
  ctx.save(); ctx.translate(0, -25); ctx.rotate(rad(-120 + 240 * fr)); ctx.fillStyle = rig.col(INK); ctx.fillRect(-1, -4, 2, 5); ctx.restore();
}
/** Brass crossbar with a dark iron hip block (hip space). */
function drawHips(ctx, rig, pose, inf) {
  const hw = R(inf.w / 2);
  celRect(ctx, rig, -hw, -4, inf.w, 9, 2, PAL.accent, 0.4, 0.25);
  ctx.beginPath(); ctx.rect(-5, -6, 10, 12); flat(ctx, rig, IRON);
  if (rig.override) return;
  ctx.fillStyle = tones(rig, IRON).hi; ctx.fillRect(-4, -5, 8, 1);
}
/** Brass ball joint on the shoulder (shoulder space, over the upper arm). */
function drawShoulder(ctx, rig, pose, inf) { celBall(ctx, rig, 0, 0, inf.r, inf.pal.accent); }
/** Upper arm: light grey cylinder with a brass band at the elbow end (limb space, +x along the arm). */
function drawArmUpper(ctx, rig, pose, inf) {
  const r = inf.r, L = inf.len;
  celRect(ctx, rig, -2, -r, L + 3, r * 2, 3, inf.pal.sleeve, 0.36, 0.28);
  if (rig.override) return;
  ctx.fillStyle = rig.col(inf.pal.accent); ctx.fillRect(L - 4, -r + 1, 3, r * 2 - 2);
}
/** Forearm: dark iron piston sleeve at the elbow, light steel rod to the wrist; the rod glows when the piston fires (rig.vent). */
function drawArmLower(ctx, rig, pose, inf) {
  const r = inf.r, L = inf.len;
  celCapsule(ctx, rig, 6, 0, L + 1, 0, r * 0.55, inf.pal.metal, 0.3);
  celRect(ctx, rig, -2, -r - 1, 10, r * 2 + 2, 2, inf.pal.secondary, 0.38, 0.25);
  if (rig.override) return;
  ctx.fillStyle = rig.col(inf.pal.accent); ctx.fillRect(6, -r, 2, r * 2 - 1);
}
/** Two-prong brass claw (hand space, origin at the wrist, +x along the forearm). Opens by rig.claw (0 closed .. 1 wide). */
function drawClaw(ctx, rig, pose, inf) {
  const pal = inf.pal;
  celRect(ctx, rig, -3, -5, 8, 10, 2, pal.accent, 0.38, 0.25);
  ...
  celPoly(ctx, rig, [3, 5, 10, 6 + g * 0.6, 19, 3 + g, 20, 1 + g, 14, 1 + g * 0.4, 8, 1], pal.accent, 0.4, 0.25);
  if (rig.override) return;
  ctx.fillStyle = tones(rig, pal.accent).deep; ctx.fillRect(-1, -3, 4, 6);
}
/** Thigh: dark iron cylinder with a brass collar (limb space). */
function drawLegUpper(ctx, rig, pose, inf) {
  const r = inf.r, L = inf.len;
  celRect(ctx, rig, -2, -r, L + 3, r * 2, 2, inf.pal.secondary, 0.38, 0.25);
  if (rig.override) return;
  ctx.fillStyle = rig.col(inf.pal.accent); ctx.fillRect(0, -r + 1, 3, r * 2 - 2);
}
/** Shin: light steel piston rod sliding out of a short dark sleeve at the knee. */
function drawLegLower(ctx, rig, pose, inf) {
  const r = inf.r + 1, L = inf.len;
  celCapsule(ctx, rig, 4, 0, L, 0, r * 0.55, inf.pal.metal, 0.3);
  celRect(ctx, rig, -2, -r, 8, r * 2, 2, inf.pal.secondary, 0.38, 0.25);
}
/** Plate foot: flat slate plate with a brass toe cap and a 2 px sole (ankle space). */
function drawPlateFoot(ctx, rig, pose, inf) {
  ...
  if (rig.override) return;
  ctx.fillStyle = rig.col(inf.pal.accent); ctx.fillRect(toe - 5, top + 1, 4, 3);
  ctx.fillStyle = tones(rig, inf.pal.dark).deep; ctx.fillRect(-heel, sole - 1, toe + heel, 2);
}
/** Cage front (torso space, front layer): two side posts, a brass base plate and a low front rail below the chin. */
function drawCageFront(ctx, rig) {
  const hw = R(rig.p.torsoW / 2), H = rig.p.torsoH;
  ctx.beginPath(); ctx.rect(-hw - 2, -H - 36, 3, 36); ctx.rect(hw - 1, -H - 36, 3, 36); flat(ctx, rig, PAL.primary);
  ctx.beginPath(); ctx.rect(-hw - 3, -H - 1, hw * 2 + 6, 3); flat(ctx, rig, PAL.accent);
  ctx.beginPath(); ctx.rect(-hw - 3, -H - 12, hw * 2 + 6, 3); flat(ctx, rig, PAL.primary);
  if (rig.override) return;
  ctx.fillStyle = rig.col(PAL.accent); ctx.fillRect(-hw - 3, -H - 38, 5, 3); ctx.fillRect(hw - 2, -H - 38, 5, 3);
}
/** Cage back + exhaust stacks + lantern (torso space, back layer). */
function drawBackGear(ctx, rig) {
  const p = rig.p, hw = R(p.torsoW / 2), H = p.torsoH;
  // back rail of the cage behind the head
  ctx.beginPath(); ctx.rect(-hw - 1, -H - 24, hw * 2 + 2, 3); flat(ctx, rig, tones(rig, PAL.primary).sh);
  // two exhaust stacks rising behind the rear post
  for (let i = 0; i < 2; i++) {
    const x = -hw - 9 + i * 6;
    celRect(ctx, rig, x, -H - 14 + i * 3, 5, 18 - i * 3, 1, IRON, 0.4, 0);
    if (!rig.override) { ctx.fillStyle = rig.col(PAL.accent); ctx.fillRect(x - 1, -H - 15 + i * 3, 7, 3); }
  }
  // lantern: brass bracket, two-link chain that lags the torso, iron lantern with a flickering window
  ctx.beginPath(); ctx.rect(-hw - 10, -14, 10, 3); flat(ctx, rig, PAL.accent);
  const ch = getChain(rig, 'lantern', 2, { joint: 'torso', rest: [0, 1], stiffness: 0.12, damping: 0.72, gain: 1.8, rotGain: 0.5, maxAng: 40 });
  ctx.save(); ctx.translate(-hw - 9, -11);
  for (let i = 0; i < 2; i++) { ctx.rotate(rad(ch.ang[i])); ctx.beginPath(); ctx.rect(-1, 0, 2, 5); flat(ctx, rig, PAL.metal); ctx.translate(0, 5); }
  celRect(ctx, rig, -4, 0, 8, 9, 1, IRON, 0.4, 0);
  if (!rig.override) {
    ctx.fillStyle = rig.col(LAMP); ctx.fillRect(-2, 2, 4, 5);
    ctx.fillStyle = rig.col(HOT); ctx.fillRect(-1, 3 + ((rig.tick >> 2) & 1), 2, 2);
    ctx.fillStyle = rig.col(PAL.accent); ctx.fillRect(-3, -1, 6, 2);
  }
  ctx.restore();
  if (rig.override) return;
  stackPuffs(ctx, rig, -hw - 6.5, -H - 16, 0);
  stackPuffs(ctx, rig, -hw - 0.5, -H - 13, 10);
}
...
const build = {
  ...
  proportions: { headR: 9, neck: 10, torsoW: 26, torsoH: 30, hip: 24, upperArm: 15, lowerArm: 14, armR: 5.5, handR: 7, upperLeg: 13, lowerLeg: 13, legR: 4.5, footL: 13, footH: 5, shoulderX: 3, hipX: 5, bulge: 0, neckR: 3 },
...
/** Select / HUD bust: the gnome in her cage, from the real rig in the idle carry. */
const portraitRig = buildRig(build);
function portrait(ctx, x, y, s) { drawHeadPortrait(ctx, portraitRig, anims.idle.frames[0].pose, x, y, s, { fill: 0.5, cy: 0.5 }); }
