...
// ({ name, hp, color, armor, unlaunchable, grabbable, walkSpeed, damageMult, build, anims, ai, overheat, hurtboxScale, phaseIndex }).
import { ST } from '../constants.js';
import { Enemy, AI_DEFAULTS } from './enemy.js';
...
export class Boss extends Enemy {
  ...
  /** Switch to phase i: merged def, rig / anims swap, hp, AI table. */
  applyPhase(i, first = false) {
    const pd = this.phaseDefs[i];
    this.phaseIndex = i;
    this.def = mergePhase(this.baseDef, pd);
    if (!first) {
      if (pd.build) {
    ...
    this.ai = { ...AI_DEFAULTS, ...this.def.ai };
    this.walkSpeed = this.def.walkSpeed || 1.2; this.runSpeed = this.walkSpeed * 1.5;
    this.armor = !!this.def.armor; this.unlaunchable = !!this.def.unlaunchable;
    this.damageMult = this.def.damageMult || 1; this.damageTaken = this.def.damageTaken || 1;
    ...
    this.pendingAttack = null; this.currentAttack = null; this.aiState = 'APPROACH'; this.staggerTimer = 0;
  }

  // ---------- per-step ----------
  update(world) {
    if (!this.announced) { this.announced = true; if (world.onBossSpawn) world.onBossSpawn(this); }
  ...
  takeHitRaw(damage, type, attacker) {
    if (this.transition > 0 || this.defeated) return;
    super.takeHitRaw(damage, type, attacker);
  ...
  nextPhase() {
    ...
    this.dead = false; this.thrownBy = null; this.juggleCount = 0; this.juggleImmune = false; this.hitstop = 0;
  ...
  beginDefeat() {
    this.defeated = true; this.dead = true; this.hp = 0;
    if (this.grabTarget) this.releaseGrab(false);
    if (this.grabbedBy) { this.grabbedBy.releaseGrab(false); this.grabbedBy = null; }
    this.thrownBy = null; this.hitstop = 0; this.invuln = 9999;
    ...
    if (this.world) { this.world.camera.shake(12, 16); this.world.addFx('flash', this.x, 0, this.z, { color: '#ffffff' }); }
  }
  onActionDone(world) {
    if (this.defeated) {
