...
import { VIEW_W, VIEW_H, FLOOR_TOP, Z_MAX, ST, UI, WAVE_EXTRA_BY_PARTY, PARTY_EXTRA_DELAY } from '../constants.ts';
import { createBackdrop, backdropsReady } from '../art/backgrounds/index.ts';
...
import { EventRunner } from './events.ts';
import { Actor, Sign } from './actors.ts';
import { Dialogue, PLATE_COOLDOWN } from './dialogue.ts';
import { STRIP_H } from './hud.ts';
import { getEnemyDef } from '../content/enemies/index.ts';
import { CHARACTERS, getCharacter } from '../content/characters/index.ts';
...
export class StageRunner {
  ...
  constructor(world: StageWorld, stage: StageData, { game, hud, screen, nowaves = false, startSection = 0, startEvent = '' }: StageRunnerOpts) {
    ...
    /** @type {import('./actors.ts').Actor[]} bodies the running beat put on stage, dropped when it ends */
    this.actors = [];
    /** @type {import('./actors.ts').Sign[]} lettered boards the running beat put up, dropped with it */
    this.signs = [];
    /** Letterbox travel in frames, 0..CINEMA_SLIDE: counts up while a beat runs and back down once it is over. */
    this.cinemaT = 0;
    /** A scheduled exchange: `{ trigger, t, focus }`, counted down in update(). See saySoon(). */
