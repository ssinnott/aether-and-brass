...
/** The seven triggers, in the order issue #25 lists them. A stage or a def may key any of these. */
 ...
 */
const RANK = Object.freeze({ partnerDown: 5, partnerContinue: 4, bossIntro: 3, midbossIntro: 3, results: 2, sectionStart: 1, combo20: 0 });
...
export class Dialogue {
   ...
   * @param {{ focus?: object }} [o] `focus` forces who speaks first (the hero who hit the combo, the one who went out)
   */
  say(trigger, speakers, frame, { focus = null } = {}) {
    ...
    const [a, b] = cast;
    const line = b ? this.pickBanter(trigger, a, b, frame) : this.pickSolo(trigger, a, frame);
  ...
  pickBanter(trigger, a, b, frame) {
    ...
    if (!rows || !rows.length) return this.pickSolo(trigger, a, frame);
    const row = rows[this.index(rows.length, frame, a)];
    return row ? { a: row.a, b: row.b } : null;
  }

  /** A hero alone (or a pairing with nothing written for this trigger) mutters to themselves. */
  pickSolo(trigger, a, frame) {
    const rows = (this.solo[a.def.id] || {})[trigger];
    if (!rows || !rows.length) return null;
    return { a: rows[this.index(rows.length, frame, a)], b: '' };
  }
   ...
   */
  index(len, frame, who) { return len <= 1 ? 0 : (((frame | 0) + (who && who.index ? who.index * 7 : 0)) % len + len) % len; }
