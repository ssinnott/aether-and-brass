...
function fingerprintPure() {
  ...
  const hexes = ['#f0c8a0', '#5a2e18', '#8a3a2a', '#3a4a6a', '#e2b34a', '#000000', '#ffffff'];
...
for (const c of CHARACTERS) out.subjects['character:' + c.id] = fingerprintSubject(c);
for (const g of ENEMY_GALLERY) {
  const key = 'enemy:' + (g.id || g.name);
  if (!out.subjects[key]) out.subjects[key] = fingerprintSubject(g);
}
