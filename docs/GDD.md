...
about the rig, not herself). Each boss and mid-boss answers with a line on entering every phase and one on going
down. It never takes the player's input and never blocks: one plate at a time, about two seconds, and a line that
would land while something else is speaking is dropped rather than queued. Lines live in
`src/content/characters/lines.js`; the mechanics are ARCHITECTURE section 7.
...
One nine-key block per keyboard player (`Q W E / A S D / Z X C` for P1, the same square three columns right
for P2), plus the two digits above each block. P1's block never changes — alone, in co-op or online.

| Action  | P1 (and every player online) | P2 (local co-op) | Gamepad |
|---------|------------------------------|------------------|---------|
| move    | W A S D (or Arrows)          | T F G H          | D-pad / stick |
| attack  | Z                            | V                | 0 (A) |
| jump    | X                            | B                | 1 (B) |
| dodge   | C                            | N                | 2 (X) |
| special | Q                            | R                | 3 (Y) |
...
Global: Escape pauses/unpauses, M mutes, F1 debug overlay. Either couch seat joins by pressing its own key or pad button at any time (drop-in). Gamepads claim the lowest free couch seat on their first button press (never index-bound). The couch seats two (`LOCAL_PLAYERS`); three or four players is an online room, seated by the lobby.
...
- **Title:** navy gradient; a brass gear outline (r 140px) rotates behind the tiered-city silhouette. Logo "**CALDERWICK**" in chunky brass rects with a 3px bevel; "*Brass & Aether*" in cyan italic; the four heroes idle on the gear. Menu: START / ONLINE CO-OP / TRAINING / BESTIARY / OPTIONS (difficulty, friendly fire, volumes, scale, controls); the BESTIARY row carries the book's completion percentage, teal once it is full; any free slot joins with its own key or pad. "PRESS START" blinks 30f.
...
- **In-game HUD (top strip 640×40, alpha 0.5):** P1 left, P2 right. 24×24 bust portrait; name; **shield strip** 120×2 in brass directly above the health bar (pale when full, dim slow pulse while down); **health bar** 120×8 (`#59C3A0` → `#F2C94C` under 50% → `#FF5C5C` under 25%; white 4f flash on damage; a "ghost" bar drains behind after 20f); **special meter** 120×5 in three segments, filling cyan, full = pulsing white rim, red tint when a special would cost health; **lives** as character icons (goggles / scarf / tricorne / red hat); **score** 7 digits. Center: stage timer and "GO →". **Combo counter** on the player's side: 24px number + grade word, scales 1.3→1.0 over 6f per hit, color climbs grey → yellow → orange → cyan → white. **Enemy health:** 40×3 bar above the head, shown 90f after each hit; elites 60×4 with an armor icon; bosses a 400×10 bottom bar with name plate and phase segments. Pickups show floating "+25" text. With three or four players — an online room — the strip becomes four 158px columns left to right (P1..P4 in slot order, name above the bar, bars filling rightward); the timer, GO arrow and target readout drop below the strip.
- **Pause:** 60% dim, brass plate: RESUME / MUTE / OPTIONS / MOVES / COMMANDS / QUIT TO TITLE, plus a join hint for every free slot (P2 key / P3-P4 any pad button) while a slot is free. MOVES opens the same move list as the training pause plate for whichever heroes are in the run; COMMANDS opens the quick reference (below). Both are hidden during online co-op, since a screen popped by only one peer's local edge would leave the two peers' screen stacks disagreeing.
- **Commands & sound:** one wide plate reached from either pause plate. Eleven rows — move, run, attack, jump, dodge, special, super, taunt, throw, pause, mute — each printing the key and the gamepad button actually bound to it (this player's own block, which never changes) next to a one-line description of what it does, so a forgotten button never costs a trip to the title. Under a rivet divider, the same MUSIC / SFX / MUTE settings the OPTIONS plate owns, adjustable in place: up/down picks a row, left/right changes it, and the change persists like any other option.
...
- **Results / ranking:** §6. A run that opened bestiary entries names them under the totals — as many as fit on one line, then a count for the rest.
- **Bestiary:** the book of everything you have beaten, reached from the title. A faction tab row (Brassbound,
