...
- **Commands & sound:** one wide plate reached from either pause plate. Eleven rows — move, run, attack, jump, dodge, special, super, taunt, throw, pause, mute — each printing the key and the gamepad button actually bound to it (the 1P arcade half until P2 joins, P1's own keys after) next to a one-line description of what it does, so a forgotten button never costs a trip to the title. Under a rivet divider, the same MUSIC / SFX / MUTE settings the OPTIONS plate owns, adjustable in place: up/down picks a row, left/right changes it, and the change persists like any other option.
