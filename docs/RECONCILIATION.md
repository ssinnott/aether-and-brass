...
- Character select: BACK from an *unlocked* slot leaves — the screen for P1, the party for anybody else. Without that exit a slot that joined and went quiet (a pad whose battery died) holds the READY gate shut for the whole room, and nothing on the screen can clear it: `input.joined` survives backing out to BOARD SELECT and returning, and only the title resets it.
